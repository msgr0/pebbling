      ...
      *p = 0;
      if (p - q >= 5 && q[2] == ':' && q[4] == ':' &&
        ...
        if (type == 'A') {
          ...
          kputc_('Z', &str);
          kputsn_(q, p - q + 1,
                  &str); // note that this include the trailing NULL
        } else if (type == 'B') {
          type = *q++; // q points to the first ',' following the typing byte
          if (p - q >= 2 &&
              (type == 'c' || type == 'C' || type == 's' || type == 'S' ||
            ...
            kputc_(type, &str);
            kputsn_(&n, 4, &str);
            // TODO: to evaluate which is faster: a) aligned array and then
            // memmove(); b) unaligned array; c) kputsn_()
            if (type == 'c')
              while (q + 1 < p) {
...
int gfa_aux_format(int l_aux, const uint8_t *aux, char **t, int *max) {
  ...
  while (s < aux + l_aux) {
    ...
    if (type == 'A') {
      ...
      for (i = 0; i < n;
           ++i) { // FIXME: for better performance, put the loop after "if"
        kputc(',', &str);
        if ('c' == sub_type) {
          kputw(*(int8_t *)s, &str);
          ++s;
...
int gfa_parse_S(gfa_t *g, char *s) {
  ...
  if (is_ok) { // all mandatory fields read
    ...
    if (LN >= 0 && len != LN && gfa_verbose >= 2)
      fprintf(stderr,
              "[W] for segment '%s', LN:i:%d tag is different from sequence "
              "length %d\n",
              seg, LN, len);
    ...
    if (l_aux) {
      ...
      if (s_L1) {
        if (ov != INT32_MAX && s_L1[0] == 'i')
          g->seg[v >> 1].len = g->seg[v >> 1].len > ov + *(int32_t *)(s_L1 + 1)
                                   ? g->seg[v >> 1].len
                                   : ov + *(int32_t *)(s_L1 + 1);
      ...
      if (s_L2) {
        if (ow != INT32_MAX && s_L2[0] == 'i')
          g->seg[w >> 1].len = g->seg[w >> 1].len > ow + *(int32_t *)(s_L2 + 1)
                                   ? g->seg[w >> 1].len
                                   : ow + *(int32_t *)(s_L2 + 1);
...
int gfa_parse_P(gfa_t *g, char *s) {
    ...
    if (*p == 0 || *p == '\t') {
      ...
      *p = 0;
      if (i == 0) {
        ...
        char *pp, *qq, *seg_start;
        // Count segments (by counting + or -)
        for (pp = q, t.n_v = 0; pp < p; ++pp)
          if (*pp == '+' || *pp == '-')
            t.n_v++;
        GFA_MALLOC(t.v, t.n_v);
        ...
        for (pp = q; pp <= p; ++pp) {
          if (*pp == ',' || *pp == 0) {
            ...
            int32_t seg;
            char ori = *(pp - 1); // Get orientation (+/-)

            if ((ori == '+' || ori == '-') && pp - seg_start > 1) {
              // Temporarily null-terminate before the orientation
              *(pp - 1) = 0;
              seg = gfa_name2id(g, seg_start);
              *(pp - 1) = ori; // Restore orientation char

              if (seg >= 0) {
                t.v[t.n_v++] = (uint32_t)seg << 1 | (ori == '-');
              } else {
                if (gfa_verbose >= 2)
                  fprintf(stderr, "WARNING: failed to find segment '%s'\n",
                          seg_start);
            ...
            if (*pp == ',') {
              seg_start = pp + 1; // Start of next segment
            }
          }
        }
        is_ok = 1, rest = c ? p + 1 : 0;
  ...
  if (is_ok) {
    ...
    if (l_aux > 0)
      ...
      free(aux);
    GFA_GROW(gfa_walk_t, g->path, g->n_path, g->m_path);
    g->path[g->n_path++] = t;
  } else
    return -1;
  return 0;
...
static void gfa_update_fa_seq(gfa_t *g, gfa_seg_t *seg, int32_t l_seq,
                              const char *seq) {
  if (seg == 0)
    return;
...
static void gfa_write_S(kstring_t *out, const gfa_t *g, const gfa_seg_t *s,
                        int flag) {
  if (s->del)
    return;
  ...
  if (s->utg && s->utg->n) {
    ...
    for (j = l = 0; j < s->utg->n; ++j) {
      ...
      gfa_sprintf_lite(out, "A\t%s\t%d\t%c\t%s\t%d\t%d\n", s->name, l,
                       "+-"[u->a[j] >> 32 & 1], u->name[j],
                       (int32_t)(u->r[j] >> 32), (int32_t)u->r[j]);
      l += (uint32_t)u->a[j];
    }
  }
}

static void gfa_write_L(kstring_t *out, const gfa_t *g, const gfa_arc_t *a,
                        int flag) {
  const gfa_aux_t *aux = a->link_id < g->n_arc ? &g->link_aux[a->link_id] : 0;
  if (a->del || a->comp)
    return;
  gfa_sprintf_lite(out, "L\t%s\t%c\t%s\t%c", g->seg[a->v_lv >> 33].name,
                   "+-"[a->v_lv >> 32 & 1], g->seg[a->w >> 1].name,
                   "+-"[a->w & 1]);
...
static void gfa_write_W(kstring_t *out, const gfa_t *g, const gfa_walk_t *w,
                        int flag) {
  int32_t j;
  gfa_sprintf_lite(out, "W\t%s\t%d\t%s\t", w->sample, w->hap,
                   g->sseq[w->snid].name);
  if (w->st >= 0 && w->en >= 0)
